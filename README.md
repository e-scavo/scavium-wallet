...
- Phase 9.3 active — Theme Token Normalization; 9.3.1 closed the token baseline/naming contract by introducing `lib/app/theme/tokens/`, preserving `AppColors`/`AppTextStyles` compatibility facades, and keeping dark-only runtime behavior unchanged. Next implementation sequence: 9.3.2 core token model hardening, 9.3.3 ThemeData/shared-widget adoption, and 9.3.4 closure.
...
- 9.3 — Theme Token Normalization (active)
...
Phase 9 is now open as the next product-maturity phase. Phase 9.0 completed the documentation lock for application identity and visual-system consistency. Phase 9.1 then implemented the runtime app version surface: `package_info_plus` resolves package metadata, `lib/core/app_identity` owns the application version boundary, Settings/About consumes that boundary through Riverpod, and focused tests prove deterministic version formatting and provider override behavior. Phase 9.2 is now closed as the compact build-version hardening sequence: 9.2.1 locked the version/MSIX contract, 9.2.2 hardened build-tool behavior, 9.2.3 added deterministic validation coverage, 9.2.4 aligned release/development documentation, and 9.2.close records the final closure after the `pubspec.yaml` MSIX layout normalization was confirmed. The closed contract remains that `pubspec.yaml` owns `version: 0.2.2+1`, `msix_config.msix_version` mirrors it as `0.2.2.1`, and `tool/build.dart` owns build-time interpretation, mutation, tag validation, and MSIX synchronization. The implemented work keeps that boundary in `tool/build.dart`, uses `test/build_tool_version_test.dart` as focused validation coverage, and does not change release publication, runtime Settings/About version display, wallet custody, assets, transactions, signing, backup, restore, diagnostics, routing, or theme behavior. The remaining Phase 9 work now continues through the active 9.3 token-normalization sequence, first-class light/dark themes, persisted theme-mode selection, and broader Settings/About alignment. Phase 9.3.1 has closed the baseline naming contract: `lib/app/theme/tokens/` owns the semantic SCAVIUM token namespace, `AppColors` and `AppTextStyles` remain compatibility facades, `AppTheme.darkTheme` consumes token names, and `test/app_theme_tokens_test.dart` locks facade mappings and compact scale behavior. The 9.3 sequence remains intentionally bounded to the app theme layer and shared visual components: it does not expose light mode yet, does not change `themeMode`, and does not reopen runtime version, build/MSIX, wallet, signing, backup, diagnostics, routing, release, or CI behavior.
